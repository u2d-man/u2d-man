...
### 概要
Web クリッピング関連プロダクトの開発と運用を2021年8月からリード。フロントエンドからインフラ、テスト自動化、DB 移行に至るまで幅広く対応し、プロジェクト推進とチーム連携にも注力。
2023年ごろにチームリーダーを離れ一エンジニアとして Web クリッピングの開発に携わる。
...
#### Web クリッピングの UI 刷新（2025年9月〜2025年12月）
- **フロントエンド**：React / TypeScript / Playwright / Vitest / Storybook / OpenAPI
- **開発機能概要**：
UI を刷新しました。デザイナーが作成したデザイン通りに全てのコンポーネントの再実装。
基本的に私含め2名で対応、チケットの作成、アサイン、スケジュール感の随時確認等も実施。
アクティブなチケット146件のうち90件を対応。

- **担当業務・技術的取り組み**：
  - 既存 UI の画面をユーザーに見せつつ、新 UI の QA を本番環境で実施するためどのように html ファイルを配信すれば良いかを他メンバーと相談しながら進行
  - QA に使用するテストケースの作成をメンバーに依頼・テストケースのレビュー
  - QA 実施
  - PdM とスケジュール感に関して何度か打ち合わせ
